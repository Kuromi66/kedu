...
当前仓库对应版本为 `v1.7.1`，已经支持正式包签名构建、历史补卡、本地通知提醒、数据导入导出、账号登录与多设备云同步、重要日期（纪念日/倒计时/按月循环/提前提醒）、归档习惯管理、打卡时间编辑、CSV 导出、桌面小组件、检查更新、中英文切换，以及今日 / 历史 / 统计 / 设置四个主要页面。

后续功能规划见 [docs/TODO.md](./docs/TODO.md)（带编号与优先级的需求清单）。
...
- 打卡与重要日期两个桌面小组件：快捷打卡、进度、倒计时，深浅色适配
- 支持查看每个习惯当天的记录列表与相对时间
- 打卡可填写备注，点击记录查看详情（时间、补卡标记、备注）
- 记录详情支持编辑打卡时间
- 支持删除单条打卡记录
- 支持新增、编辑、删除习惯
- 设置页可查看已归档习惯，支持恢复与彻底删除（含关联打卡记录）
...
- 支持手动触发同步并显示上次同步时间
- 支持检查更新：手动检查弹窗展示发布说明并跳转下载，启动与每日后台检查发现新版本时推送通知
- 退出登录有二次确认，登录 / 注册时有等待动画
- 支持浅色 / 深色 / 跟随系统主题
- 支持中文 / 英文界面切换
- 支持本地 JSON 导出与导入
- 支持导出 CSV（含表头与备注，Excel 可直接打开）
- 支持通知权限状态查看与本地提醒开关
- 显示当前应用版本信息
...
- 支持按月重复（交房租、发工资等固定日期，小月自动取当月月末）
- 支持提前提醒（日期前 1-7 天本地通知，循环日期每周期自动续排）
...
1. 升级版本号：修改 `app/build.gradle.kts` 的 `versionCode` / `versionName`
2. 更新版本清单：修改 `cloud/src/index.ts` 顶部的 `VERSION_MANIFEST`（versionCode、versionName、发布说明、下载页 URL）
3. 部署后端：`cd cloud && npm run deploy`
4. 构建发布：执行 `assembleRelease` 与 `bundleRelease`，把 APK / AAB 上传到 GitHub Release（tag 与版本号一致）
...
- 云同步性能优化：日常同步改为只上传增量记录；同步请求体 gzip 压缩；服务端批量写入，显著降低同步耗时
- 新增对帐机制：每 12 小时后台任务与进入前台时，按 id+版本双向比对补全，时钟漂移也能兜底纠偏
- 修复：首条重要日期因 `sortOrder=0` 等默认字段缺失被服务端静默丢弃、同步不到云端的问题
- 修复：Cloudflare 对 chunked+gzip 请求体贴无法解析的问题（改用 Content-Length 传输）
- 登录自动清理过期与超量会话，防止云端 session 无限堆积（保留多设备登录）

### v1.7.0

- 新增检查更新：设置页手动检查；启动（每日一次）与每日后台任务静默检查，发现新版本弹窗/通知提醒，跳转浏览器下载
- 云端每周自动清理过期的彻底删除墓碑，同步数据长期保持整洁

### v1.6.0

- 新增两个桌面小组件：打卡小组件（今日概览、快捷打卡）与重要日期小组件（倒计时列表），数据变化自动刷新并适配深色模式

### v1.5.0

- 打卡记录支持编辑时间（今日页与历史页记录详情）
- 设置页新增导出 CSV（UTF-8 带 BOM，Excel 打开不乱码）

### v1.4.0

- 归档习惯管理：设置页可查看已归档习惯，支持恢复与彻底删除（习惯与其打卡记录以墓碑同步到各设备）

### v1.3.0

- 重要日期支持提前提醒（1-7 天，固定上午 9 点通知，循环日期每周期自动续排）

### v1.2.2

- 重要日期新增按月重复（每月固定日循环，小月自动回退到月末）

### v1.2.1

- 修复云同步推送：改为全量推送本地记录，避免时间戳落后的记录无法上传
- 时钟偏移持久化，App 重启后不再回退记录时间戳
